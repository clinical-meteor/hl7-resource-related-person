...
Router.map(function(){
  this.route('relatedPersonsTablePage', {
    path: '/table/relatedPersons',
    template: 'relatedPersonsTablePage'
  });
});
...
Template.relatedPersonsTablePage.events({
  'click .checkbox': function(event, template){
    event.stopPropagation();
    if(this.checked){
      RelatedPersons.update({_id: this._id}, {$set: {checked: false}});
    }else{
      RelatedPersons.update({_id: this._id}, {$set: {checked: true}});
    }
  },
  'click .flag': function(event, template){
    event.stopPropagation();
    if(this.flagged){
      RelatedPersons.update({_id: this._id}, {$set: {flagged: false}});
    }else{
      RelatedPersons.update({_id: this._id}, {$set: {flagged: true}});
    }
  },
  ...
  'click .delete': function(){
    RelatedPersons.remove(this._id);
  },
  'click tr': function(){
...
Template.relatedPersonsTablePage.helpers({
  getName: function(){
...
Template.relatedPersonsTablePage.rendered = function(){
  Template.appLayout.layout();

  // step A:  initialize the table sorting functionality
  $(this.find('#relatedPersonsTable')).tablesorter();
  ...
  Tracker.autorun(function() {
    ...
    setTimeout(function() {
      ...
      $("#relatedPersonsTable").trigger("update");
    }, 200);
  });
